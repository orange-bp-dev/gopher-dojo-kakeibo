...
func (ab *AccountBook) AddItem(item *Item) error {
	...
	// TODO: 追記用でファイルを開く
	// Createにすると、Itemを追加するたびにファイルが再作成されるので、
	// 最後に追加したItemしか残らなくなる。
	// file, err := os.Create("accountbook.txt")
	file, err := os.OpenFile(ab.fileName,  os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600)
	...
	// 「品目 値段」の形式でファイルに出力する
	if _, err := /* TODO: fmt.Printlnを使って出力 */ fmt.Fprintln(file, item.Category, item.Price); err != nil {
		return err
	}
